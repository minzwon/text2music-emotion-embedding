...
class MyDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        vads = self.song_vads[int(ix)]

        # get binaries
        binary = self.song_binaries[int(ix)]
        return song.astype('float32'), vads.astype('float32'), binary.astype('float32')
